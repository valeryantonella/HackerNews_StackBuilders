...
describe("When and array of news is valid", () => {
  test("returns an array with the news sorted by comments", () => {
    expect(EntryController.sortByComments(testData)).toEqual([
      {
...
describe("When and array of news is empty", () => {
  test("returns an empy array", () => {
    expect(EntryController.sortByComments([])).toEqual([]);
  });
});
